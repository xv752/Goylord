...
```
Goylord-Server/
├── package.json                     # v0.0.5, Bun runtime, all dependencies
...
│   │   ├── user-schema.ts           # User DB migrations (incl. 015: onboarding_completed_at)
...
│   ├── httpHandlers.ts              # REST client listing endpoint, command dispatch, markOnline/markOffline
│   ├── paths.ts                     # Path resolution (simplified resolveDataDir)
...
│   │   ├── turn-credentials.ts     # TURN/Coturn ICE credential generation
...
│       ├── remotedesktop.js         # Remote desktop (diagnostics HUD, stream stats, bitrate control)
│       ├── webrtc-stats.js          # WebRTCStatsSampler class for connection quality metrics
...
- **WebRTC**: P2P or relayed streaming for remote desktop/audio, TURN/Coturn ICE relay support
...
- **HEVC Codec Negotiation**: Browser-side decoder probing and codec capability exchange via `desktop-codec-negotiation.ts`
- **Viewer Authorization**: Per-viewer session validation and access control via `viewer-authorization.ts`
- **Simple Theme Builder**: Configurable UI theme customization in settings (colors, layout)
- **H264 Bitrate Control**: Server-side `desktop_set_bitrate` command type for dynamic bitrate adjustment
- **Stream Stats**: Receives `desktop_stream_stats` messages from agents (frame rate, bitrate, resolution)
