...
- These features were explicitly removed by the maintainer — do NOT re-add them:
  - HTTP plugin URL rejection
  - `taskkill` name restrictions
  - File upload directory restrictions
  - Requiring `AGENT_TOKEN` at startup (agents must be allowed to start without one)
